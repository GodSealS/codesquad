...
export const GrepTool = buildTool({
    ...
    validateInput(input, context) {
        ...
        if (input.path) {
            ...
            if (!existsSync(searchPath) && context.aicoreDir && !input.path.startsWith('..')) {
    ...
    async call(input, context) {
        ...
        if (!existsSync(searchRoot) && input.path && context.aicoreDir && !input.path.startsWith('..')) {
            const relPath = input.path.replace(/^AICore[\\/]/, '');
...
export const GlobTool = buildTool({
    ...
    validateInput(input, context) {
        if (input.path) {
            ...
            if (!existsSync(base) && context.aicoreDir && !input.path.startsWith('..')) {
                const relPath = input.path.replace(/^AICore[\\/]/, '');
                const aicorePath = resolve(context.aicoreDir, relPath);
                if (existsSync(aicorePath))
                    base = aicorePath;
    ...
    async call(input, context) {
        ...
        if (!existsSync(cwd) && input.path && context.aicoreDir && !input.path.startsWith('..')) {
            const relPath = input.path.replace(/^AICore[\\/]/, '');
