...
/* ── CODESSQUAD.md engine templates ──────────────── */
...
/* ── config operations ─────────────────────────── */
function readCodessquadMd(targetPath) {
    const p = join(targetPath, 'CODESSQUAD.md');
    return existsSync(p) ? readFileSync(p, 'utf-8') : null;
}
function updateCodessquadMd(targetPath, engine, version) {
    const p = join(targetPath, 'CODESSQUAD.md');
    ...
    logger.success('Updated CODESSQUAD.md');
}
...
export async function runSetupEngine(options = {}) {
    ...
    // 4. Update CODESSQUAD.md
    const codesquadExists = existsSync(join(targetPath, 'CODESSQUAD.md'));
    if (codesquadExists) {
        updateCodessquadMd(targetPath, engine, version);
    }
    else {
        logger.info('CODESSQUAD.md not found — skipping update.');
    }
