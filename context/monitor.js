...
// ── Stats ──
let _lastCompactionTime = null;
export function recordCompaction() {
    _lastCompactionTime = new Date().toISOString();
}
export function resetCompactionRecord() {
    _lastCompactionTime = null;
}
/**
 * Calculate context usage statistics.
 */
export function calculateContextStats(messages, model) {
    const window = getContextWindow(model);
    const tokenCount = estimateTokenCount(messages, model);
    const total = tokenCount.total;
    const percentUsed = (total / window) * 100;
    return {
        model,
        ...
        lastCompaction: _lastCompactionTime || undefined,
    };
}
/**
