...
export async function runAgent(config) {
    ...
    let agentPrompt;
    try {
        agentPrompt = readFileSync(agentPath, 'utf-8');
    }
    catch {
    ...
    try {
        ...
        while (turn < maxTurns) {
            ...
            if (useStreaming) {
                ...
                try {
                    ...
                    console.warn('[AgentRunner] Stream failed, falling back to non-streaming');
        ...
        const msg = err instanceof LlmError ? err.message : `Agent call failed: ${err.message}`;
        onError?.(msg);
