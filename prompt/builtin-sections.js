...
// ── Cache: project guidance loaded once ──
let _projectGuidanceCache = null;
/**
 * Feature 6 (P4): Recursive CODESSQUAD.md discovery.
 * Walks from cwd up to filesystem root, collecting CODESSQUAD.md files.
 * Also supports .codesquad/CODESSQUAD.md, codesquad.local.md, and @include directives.
 *
 * Load order (lowest to highest priority):
 *   1. Managed:   ~/.codesquad/CODESSQUAD.md + ~/.codesquad/rules/*.md
 *   2. Ancestor:  codesquad.md in each parent directory (root → cwd)
 *   3. Project:   .codesquad/CODESSQUAD.md (at project root)
...
function discoverCodesquadFiles(projectRoot, extraDirs) {
    ...
    // 1. Managed: ~/.codesquad/CODESSQUAD.md + rules/*.md
    try {
        const managedPath = join(CODESQUAD_USER_ROOT, 'CODESSQUAD.md');
    ...
    // 3. Project-level: .codesquad/CODESSQUAD.md
    const projectConfig = join(projectRoot, '.codesquad', 'CODESSQUAD.md');
...
function loadProjectGuidance(projectRoot, extraDirs, bare) {
    if (_projectGuidanceCache)
        return _projectGuidanceCache;
    const parts = [];
    // Feature 6 (P4): Recursive CODESSQUAD.md discovery (unless --bare)
    if (!bare) {
    ...
    // Read CODESSQUAD.md from project locations (rule 4), then fall back to CLI's AICore template (rule 3).
    // Priority: .codesquad/CODESSQUAD.md → project root CODESSQUAD.md → AICore/CODESSQUAD.md (CLI template)
    const dotCodesquadMd = join(projectRoot, '.codesquad', 'CODESSQUAD.md');
    const projectRootMd = join(projectRoot, 'CODESSQUAD.md');
    const codebuddyMd = join(projectRoot, 'CODEBUDDY.md');
    let codessquadFound = false;
    for (const p of [dotCodesquadMd, projectRootMd]) {
        try {
            if (existsSync(p)) {
                parts.push(readFileSync(p, 'utf-8'));
                codessquadFound = true;
                break;
            }
        }
        catch { /* try next */ }
    ...
    if (!codessquadFound) {
        const cliTemplate = join(projectRoot, 'AICore', 'CODESSQUAD.md');
        try {
    ...
    _projectGuidanceCache = parts.join('\n\n---\n\n');
    return _projectGuidanceCache;
}
export function invalidateProjectGuidance() {
    _projectGuidanceCache = null;
}
// ── Section Definitions ──
/**
...
/**
 * Project guidance — CODESSQUAD.md + CODEBUDDY.md.
 */
