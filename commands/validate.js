...
async function handleValidateAudit() {
    ...
    console.log(chalk.bold('\n── Coverage Report ──'));
    console.log(`  Skills:  ${stats.skillsTested}/${stats.totalSkills} tested (${((stats.skillsTested / stats.totalSkills) * 100).toFixed(1)}%)`);
    console.log(`  Agents:  ${stats.agentsTested}/${stats.totalAgents} tested (${((stats.agentsTested / stats.totalAgents) * 100).toFixed(1)}%)`);
