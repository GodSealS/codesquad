...
/**
 * Load project-level guidance from CODESSQUAD.md (CLI config),
 * CODEBUDDY.md (IDE config), and AICore/CODESSQUAD.md (system template).
 * Priority: .codesquad/CODESSQUAD.md → project root CODESSQUAD.md → CLI template
 * Mirrors Claude Code's loadMemoryPrompt() which reads CLAUDE.md + memory directory.
 */
function loadProjectGuidance() {
    const parts = [];
    // Rule 4: Read CODESSQUAD.md from project locations first (dynamic, user-modifiable)
    const dotCodesquadMd = join(PROJECT_ROOT, '.codesquad', 'CODESSQUAD.md');
    const projectRootMd = join(PROJECT_ROOT, 'CODESSQUAD.md');
    ...
    // Rule 3: Fallback to CLI's AICore template (backward compat)
    if (parts.length === 0) {
        const cliTemplate = join(AICORE_DIR, 'CODESSQUAD.md');
        try {
...
function agentExists(agentName) {
    return existsSync(join(AICORE_DIR, 'agents', `${agentName}.md`));
}
// ── REPL loop ──
export async function startRepl() {
    ...
    const state = {
        currentSession: null,
        providerId: null,
        modelId: flagDefaultModel ? null : null,
    ...
    if (cliFlags.bare) {
        console.log(infoLine('Bare mode: CODESSQUAD.md 自动发现已跳过。'));
    }
    ...
    async function handleBuiltinCommand(cmd) {
        switch (cmd.name) {
            ...
            // ── Task management (Feature 2, P4) ──
            case 'tasks':
                await handleTasks(cmd.args);
                break;
            // ── Team management (Feature 3, P4) ──
            case 'team':
                await handleTeam(cmd.args);
                break;
            default:
        ...
        try {
            while (turn < MAX_TURNS) {
                ...
                const ctxTokens = response.usage
                    ? Math.round((response.usage.totalTokens / 200000) * 100)
                    : null;
