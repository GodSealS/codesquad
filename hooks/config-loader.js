...
import { getCodeSquadProjectCategory, getCodeSquadUserCategory, CODESQUAD_USER_SETTINGS, isEmbeddedMode, readAicoreFile } from '../core/paths.js';
...
export function loadHooksFromLayered(aicoreDir, cwd) {
    ...
    for (let i = 0; i < sources.length; i++) {
        ...
        // ── Embedded mode: Layer 0 (AICore built-in) read from memory ──
        if (isEmbeddedMode() && i === 0) {
            const raw = readAicoreFile('settings.json');
            if (!raw)
                continue;
            try {
                const settings = JSON.parse(raw);
