...
async function executeCommandHook(hook, input) {
    ...
    const shell = hook.shell || (process.platform === 'win32' ? 'powershell.exe' : '/bin/sh');
    ...
    const escapedCommand = hook.command.replace(/"/g, '\\"');
    const shellCommand = process.platform === 'win32'
        ? `powershell.exe -Command "${escapedCommand}"`
    ...
    try {
        ...
        // On success, execAsync resolves with { stdout, stderr } — no code property.
        // Successful exit is always code 0.
        return {
            available: true,
        ...
        // ENOENT (command not found): hook script is missing → allow (safe default).
        // Timeout (err.killed): block to prevent hanging tools.
        // Other errors: treat as block (broken hook is safer than silent bypass).
        if (err.code === 'ENOENT') {
            return {
                available: true,
                decision: 'allow',
        ...
        const exitCode = err.killed ? 2 : (err.code ?? 1);
        return {
            available: true,
            decision: parseHookExitCode(exitCode),
            stdout: err.stdout?.trim(),
            stderr: err.stderr?.trim(),
            exitCode,
            error: err.killed ? `Command timed out after ${timeout}s` : err.message,
        };
    }
}
/**
