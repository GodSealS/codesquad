...
import { CLI_PACKAGE_ROOT } from '../../core/paths.js';
// ── Skill Loading ──
const CODEBUDDY_DIR = '.codebuddy';
/** Fallback directory for MCP bridge stubs (published version uses this path) */
const MCP_STUBS_SKILLS_DIR = join(CLI_PACKAGE_ROOT, '.aicore-mcp-stubs', 'skills');
/**
 * Resolve the skill SKILL.md path, checking multiple locations in order:
 * 1. ${projectRoot}/.codebuddy/skills/${name}/SKILL.md (dev / standard install)
 * 2. ${CLI_PACKAGE_ROOT}/.aicore-mcp-stubs/skills/${name}/SKILL.md (bridge stubs)
 */
function resolveSkillPath(projectRoot, skillName) {
    const primary = join(projectRoot, CODEBUDDY_DIR, 'skills', skillName, 'SKILL.md');
    if (existsSync(primary))
        return primary;
    const fallback = join(MCP_STUBS_SKILLS_DIR, skillName, 'SKILL.md');
    if (existsSync(fallback))
        return fallback;
    throw mcpError(McpErrorCode.SKILL_NOT_FOUND, `Skill not found: ${skillName}. Checked:\n  - ${primary}\n  - ${fallback}`);
}
/** Load skill prompt template from the resolved skill path */
