 ...
 */
import { writeFile, readFile, mkdir, rename } from 'fs';
...
/** Lazily resolve usage directory so setUsageProjectRoot() can take effect after module load. */
function getUsageDir() {
    return join(usageHome(), 'usage');
}
// ── Path helpers ──
...
function usageFilePath(monthKey) {
    return join(getUsageDir(), `${monthKey}.json`);
}
async function ensureUsageDir() {
    await mkdirAsync(getUsageDir(), { recursive: true });
}
// ── Calculate cost ──
...
export async function recordUsage(record) {
    ...
    records.push(record);
    // Atomic write to prevent concurrent write corruption
    const tmpPath = filePath + '.tmp';
    await writeFileAsync(tmpPath, JSON.stringify(records, null, 2));
