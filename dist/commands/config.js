    ...
    switch (action) {
        case 'show': {
            ...
            const hasContent = (Object.keys(mc.agents ?? {}).length > 0)
                ...
                || (mc.default !== null && mc.default !== undefined)
        ...
        case 'set': {
            ...
            // Strip matching outer quotes from model name (handles multi-word names like "model name")
            const model = modelParts.join(' ').replace(/^["'](.+)["']$/, '$1');
            if (pathStr === 'default') {
