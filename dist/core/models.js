...
function deepCloneConfig(cfg) {
    return {
        version: cfg.version,
        agents: cfg.agents
            ? Object.fromEntries(Object.entries(cfg.agents).map(([k, v]) => [k, deepCloneOverride(v)]))
            : undefined,
        skills: cfg.skills
            ? Object.fromEntries(Object.entries(cfg.skills).map(([k, v]) => [k, deepCloneOverride(v)]))
            : undefined,
...
/** Deep-clone a ModelOverride (string or {model, source}) */
function deepCloneOverride(ov) {
    if (typeof ov === 'string')
        return ov;
    return { ...ov };
}
...
export function computeToolDefaultModels(toolIds) {
    ...
    for (const toolId of toolIds) {
        ...
        // Right-hand spread (merged.*) wins → earlier tools' values are preserved
        if (defaults.batch)
